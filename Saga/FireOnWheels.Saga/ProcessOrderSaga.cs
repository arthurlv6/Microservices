...
namespace FireOnWheels.Saga
{
    public class ProcessOrderSaga: Saga<ProcessOrderSagaData>,
        ...
        IHandleMessages<IEmailSentMessage>
    {
        ...
        public void Handle(IOrderDispatchedMessage message)
        {
            Logger.InfoFormat("----------------->Order {0} has been dispatched.",
                Data.OrderId);
            Bus.Send(new EmailCommand());
        }

        public void Handle(IEmailSentMessage message)
        {
            Logger.InfoFormat("----------------->Email has been sent and ending saga.",
            Data.OrderId);
            ReplyToOriginator(new OrderProcessedMessage() { Price = 123 });
            MarkAsComplete();
        }
    }
}
